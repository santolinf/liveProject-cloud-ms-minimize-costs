...
import com.twa.flights.api.provider.beta.connector.configuration.CatalogConnectorConfiguration;
import com.twa.flights.api.provider.beta.dto.CityDTO;
import com.twa.flights.api.provider.beta.exception.TWAException;
...
public class CatalogConnector {
    ...
    public CityDTO getCityByCode(String code) {
        ...
        WebClient client = WebClient.builder().defaultHeader(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON_VALUE)
                .defaultHeader(HttpHeaders.ACCEPT_ENCODING, "gzip").clientConnector(connector)
                .filter(ConnectorFilter.logRequest()).filter(ConnectorFilter.logResponse()).build();
